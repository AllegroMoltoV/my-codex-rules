...
function Get-CodexRulesBytes {
    ...
    if (-not [System.IO.File]::Exists($LocalPath) -or (Get-Item -LiteralPath $LocalPath).Length -eq 0) {
        return ,([System.IO.File]::ReadAllBytes($CommonPath))
    }

    $readBody = {
        param([string]$Path, [string]$Heading)
        $content = [System.IO.File]::ReadAllText($Path).Replace("`r`n", "`n")
        if ($content -eq $Heading) {
            return ''
        }
        if ($content.StartsWith($Heading + "`n", [System.StringComparison]::Ordinal)) {
            $content = $content.Substring($Heading.Length + 1)
        }
    ...
    $common = & $readBody $CommonPath '# AGENTS.md'
    $local = & $readBody $LocalPath '# AGENTS.local.md'
    $combined = "<!-- generated by my-codex-rules -->`n`n" + $common + "`n`n" + $local
    return ,([System.Text.UTF8Encoding]::new($false).GetBytes($combined))
}
