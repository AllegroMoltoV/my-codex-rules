...
**残りは Codex が行います。** `$project-bootstrap` が Git、除外設定、作業用ディレクトリ、文書索引、Beads を初期設定します。その後、要件に関係する現状を調査し、複数セッションへまたがる作業を Beads へ登録します。このスキルは状態を変更するため暗黙起動を無効にしています。Codex への依頼文に `$project-bootstrap` を含める必要があり、INIT 内の記載だけでは明示起動になりません。
